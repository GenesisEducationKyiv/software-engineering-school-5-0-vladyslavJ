...
class SubscriptionService {
	async subscribe(
		...
		logger.info(
			`Trying to subscribe: email=${email}, city=${city}, frequency=${frequency}`
		);
		...
		logger.info(
			`Subscription created: email=${email}, city=${city}, frequency=${frequency}`
		);
	...
	async confirm(rawToken: string): Promise<void> {
		...
		try {
			...
			logger.error(
				`Error saving subscription confirmation: email=${sub.email}`,
				err
			);
			if (err instanceof QueryFailedError) {
