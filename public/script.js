...
cityInput.addEventListener('blur', async function () {
  ...
  if (exists === false) {
    resultDiv.textContent = 'City not found. Please enter a different name.';
...
document.getElementById('subscribeForm').addEventListener('submit', async function (e) {
  ...
  if (exists === null) {
    resultDiv.textContent = 'Failed to verify city. Please try again later.';
    resultDiv.className = 'error';
    return;
  }
  ...
  try {
    ...
    if (res.ok) {
      resultDiv.textContent = data.message || 'Subscription successful. Confirmation email sent.';
